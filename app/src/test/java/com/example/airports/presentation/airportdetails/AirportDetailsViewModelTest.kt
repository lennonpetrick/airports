...
internal class AirportDetailsViewModelTest {
    ...
    fun `When fetching the airport details, then it is returned`() {
        ...
        whenever(useCase.get(id)).thenReturn(Single.just(GetAirportDetailsUseCase.Result.Value(airport, nearestAirport)))
    ...
    fun `When no airport details is found, then an error is returned`() {
        val id = "AAA"
        whenever(useCase.get(id)).thenReturn(Single.just(GetAirportDetailsUseCase.Result.Empty))
    ...
    fun `When fetching the details fails, then an error is returned`() {
        val id = "AAA"
        whenever(useCase.get(id)).thenReturn(Single.error(Throwable()))
