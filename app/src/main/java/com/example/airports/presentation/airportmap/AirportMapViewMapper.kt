...
internal class AirportMapViewMapper @Inject constructor() {
    ...
    fun convert(airport: Airport, furthestAirports: Pair<Airport, Airport>?): AirportMapView {
        val furthest = furthestAirports?.let { it.first.id == airport.id || it.second.id == airport.id } ?: false
        return AirportMapView(airport.id, airport.latitude, airport.longitude, furthest)
    }
